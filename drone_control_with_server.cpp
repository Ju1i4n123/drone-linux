...
// Compile: g++ -std=c++17 -O2 -o drone_control drone_control.cpp -lpthread -lrt -lnlohmann_json
...
int main() {
    ...
    try {
        ...
        // Create drone server
DroneServer server(8080);

// Set command handler
server.setCommandHandler([&fc](const json& cmd) {
...
// Set status provider
server.setStatusProvider([&fc]() {
    json status;
    status["armed"] = fc.getState().armed;
    status["in_flight"] = fc.getState().in_flight;
    status["altitude"] = fc.getState().z;
    status["battery_voltage"] = fc.getState().battery_voltage;
    status["gps_fix"] = fc.getGPSData().fix;
    status["roll"] = fc.getState().roll * RAD_TO_DEG;
    status["pitch"] = fc.getState().pitch * RAD_TO_DEG;
    status["yaw"] = fc.getState().yaw * RAD_TO_DEG;
    return status;
});
    ...
    std::cout << "Auth token: " << server.getAuthToken() << std::endl;
